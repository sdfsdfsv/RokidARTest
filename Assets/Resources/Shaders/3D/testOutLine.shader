...
Shader "Custom/testOutLine " { // 逐像素高光反射
    Properties{
        _Diffuse("Diffuse Color", Color) = (1,1,1,1) // 可在编辑器面板定义材质自身色彩
        _Outline("_Outline Color", Color) = (1,1,1,1) // 可在编辑器面板定义材质自身色彩
        _OutlineWidth("_Outline width", Range(0,1)) = 0.1
        _Gloss("Gloss", Range(1,200)) = 10 // 高光的参数
    }
    SubShader{
        Pass {
...
#pragma fragment frag
            ...
            fixed4 frag(v2f f) : SV_Target 
            {
                ...
                float strength = max(0, dot(normalDir, lightDir));
                strength = 1;
                
                // 漫反射Diffuse颜色 = 直射光颜色 * max(0, cos(光源方向和法线方向夹角)) * 材质自身色彩
                fixed3 diffuse = _LightColor0 * strength * _Diffuse.rgb; // 颜色融合用乘法
                ...
                strength = max(dot(normalDir, halfDir), 0);
                strength = floor(strength*5)/5;
                fixed3 specular = _LightColor0.rgb * pow(strength , _Gloss);
                
                // 最终颜色 = 漫反射 + 环境光 + 高光反射
                fixed3 tempColor = diffuse + ambient + specular; // 颜色叠加用加法（亮度通常会增加）

                float d = abs(dot(normalDir , viewDir));
                if( d < _OutlineWidth ){
                    tempColor = _Outline.rgb;
                }
