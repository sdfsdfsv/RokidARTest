    ...
    SubShader{
        Pass {
            ...
            struct a2v
            {
                ...
                float4 normal : NORMAL;      // 告诉Unity把模型空间下的法线方向填充给normal属性
            ...
            v2f vert(a2v v)
            {
                ...
                f.worldNomal = mul(v.normal, unity_WorldToObject).xyz; // 反过来相乘就是从模型到世界，否则是从世界到模型
                f.worldVertex = mul(v.vertex, unity_WorldToObject).xyz;
                f.worldTangent = mul(v.tangent, unity_WorldToObject).xyz;
                return f;
            }
            ...
            fixed3 getDissolvedColor (v2f f){
